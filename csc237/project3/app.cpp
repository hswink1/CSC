...
/**
* \file app.cpp
* \brief This program reads a file into polynomials that are held in
* user-selected containers, to then be printed or evaluated by an user input
* for 'x'.
**/
...
//Universal
/**
* \brief Launches all outputs and evaluations for every class.
* \param filename - IMPORT - the file to be read.
* \param chosenNumber - IMPORT - the number used in evaluation.
* \return N/A
**/
void batchTest(const string&, const double&);

/**
* \brief User enters the file to be read.
* \param ifs - IMPORT/EXPORT - the input stream.
* \return bool - returns stream opening success.
**/
bool fileEntry(ifstream&, string&);

/**
* \brief User chooses one of four tasks to be performed.
* \param N/A
...
/**
* \brief Universal function for evaluation of P(x).
* \param ThePoly - IMPORT - the pointer to the container which holds
*        the Terms.
* \return double - the value of P(x).
**/
double evalSelect(const TermList*);

//Situational
/**
* \brief Holds the function routine for the TermArrayList options.
* \param filename - IMPORT - the file to be read.
* \return int - used to return 0 to break the infinite loop.
**/
int TArrayLOption(const string&);

/**
* \brief User chooses the function to be performed from TArrayLOption
* \param N/A
* \return int - the choice made.
**/
int TArrayLSelect();

/**
* \brief Holds the function routine for the TermVectorList options.
* \param filename - IMPORT - the file to be read.
* \return int - used to return 0 to break the infinite loop.
**/
...
/**
* \brief User chooses the function to be performed from TVectorLOption
* \param N/A
* \return int - the choice made.
**/
int TVectorLSelect();
...
/**
* \brief Holds the function routine for the TermDblLinkList options.
* \param filename - IMPORT - the file to be read.
* \return int - used to return 0 to break the infinite loop.
**/
...
/**
* \brief User chooses the function to be performed from TDblLinkLOption
* \param N/A
* \return int - the choice made.
**/
int TDblLinkLSelect();
...
/**
* \brief User is able to create data containers filled with polynomials from
* a file and display them through various print routines.
* \param argc - IMPORT - The number of arguments entered.
* \param argv - IMPORT - The values of the arguments that were passed.
* \return int - Used to close the program.
**/
  ...
  while(true){  //Loop works until cancelled
    ...
    switch(choice){
      ...
      case 2: ThePoly->printPtr();          //The list is printed by a pointer
              evalSelect(ThePoly);          //The Terms are evaluated
              break;
      case 3: ThePoly->printRecursively();  //The list is printed recursively
              evalSelect(ThePoly);          //The Terms are evaluated
              break;
      case 4: evalSelect(ThePoly);          //The Terms are evaluated
              break;
      case 5: return 0;                     //Exits to the menu in main()
              break;
    }
  }
}
...
int TArrayLSelect(){
  ...
  cout<<"3. Display all the terms in a recursive fashion."<<endl;
  cout<<"4. Evaluate the polynomials (P(x))."<<endl;
  cout<<"5. Choose another data structure."<<endl;
  cout<<"*******************************************************"<<endl;
  cout<<"Selection: ";
  cin>>choiceMade;                    //User chooses a function to be performed
  while(choiceMade>5||choiceMade<1){  //If it is not a valid function,
    cin.clear();
    cin.ignore(10000, '\n');
    choiceMade=0;
  ...
  while(true){  //Loop works until cancelled
    ...
    switch(choice){
      case 1: ThePoly->printIteratively();  //The list is printed by an iterator
              evalSelect(ThePoly);          //The Terms are evaluated
              break;
      case 2: ThePoly->printRecursively();  //The list is printed recursively
              evalSelect(ThePoly);          //The Terms are evaluated
              break;
...
int TVectorLSelect(){
  ...
  cout<<"1. Display all the terms in an iterative fashion."<<endl;
  cout<<"2. Display all the terms in a recursive fashion."<<endl;
  ...
  while(true){  //Loop works until cancelled
    ...
    switch(choice){
      case 1: ThePoly->printIteratively();  //The list is printed by an iterator
              evalSelect(ThePoly);          //The Terms are evaluated
              break;
      case 2: ThePoly->printRecursively();  //The list is printed by recursion
              evalSelect(ThePoly);          //The Terms are evaluated
              break;
      case 3: evalSelect(ThePoly);          //The Terms are evaluated
              break;
