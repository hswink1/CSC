...
{	/*********************************************************************
	*   Function name:  operator==
	*   Description:  Compares a Term's exponent against the exponent
	*   Parameters: Term right- the Term to compare the exponent by
	*
	*   Return Value: true- if match // false- if doesn't match
	*********************************************************************/
...
{ /*********************************************************************
	...
	*   Parameters: Term right- the Term to compare the exponent by
	*
	*   Return Value: true- if not match // false- if match
	*********************************************************************/
	return(getExponent()!=right.getExponent());
}

Term& Term::operator+= (const Term &right) //Adds two Terms together
{	/*********************************************************************
	*   Function name:  operator+=
	*   Description:  Fuses the coefficients of two terms, assumed to be
	*									of the same exponent.
	*   Parameters: int value - IMPORT - the value to add
	*
	*   Return Value: bool- according to the comparison
	*********************************************************************/
	return(*this=Term(getCoefficient()+right.getCoefficient(), getExponent()));
}
...
{	/*********************************************************************
	...
	*********************************************************************/
	...
	else if (term.getExponent() > 1 || term.getExponent() < 0)
